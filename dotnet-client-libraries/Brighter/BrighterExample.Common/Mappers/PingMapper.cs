...
public class PingEventMapper : IAmAMessageMapper<PingEvent>
{
    ...
    {
        var header = new MessageHeader(messageId: request.Id, topic: "ping.event", messageType: MessageType.MT_EVENT);
    ...
    {
        var pingCommand = JsonSerializer.Deserialize<PingEvent>(message.Body.Value, JsonSerialisationOptions.Options);
