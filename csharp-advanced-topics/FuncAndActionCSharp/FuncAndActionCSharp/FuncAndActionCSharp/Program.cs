...
if (input == "delegate")
{
    var delegateIntroduction = new DelegateIntro();
...
else if (input == "Action")
{
    ...
    Action<string> infoLogger = (message) => Console.WriteLine($"Additional information: {message}");
    Log.Information(infoLogger, "The request was intercepted by filtering middleware.");

    //Direct reference to a method. Log error level messages.
    Action<string> errorLogger = Log.Error;
    ...
    Log.Warning(() => Console.WriteLine($"The operation succeeded with a warning: The request was intercepted by filtering middleware"));
}
else
{
