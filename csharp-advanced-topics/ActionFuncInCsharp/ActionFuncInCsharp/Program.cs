namespace ActionFuncInCsharp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var student_1 = StudentService.GetStudent(students, (s) => s.Id == 1);
            Print<Student>(student_1, (s) => Console.WriteLine($"Id: {s.Id}  Name: {s.Name}"));

            // get student with a name: Rahul
            var student_Rahul = StudentService.GetStudent(students, (s) => s.Name.Equals("Rahul"));

            // create an Action<Student> variable and assign a method
            Action<Student> sendNotifications = NotifyAdmin;
            // add another method the sendNotifications
            sendNotifications += NotifyAccount;

            // call EnrollStudentToCourse passing student, course and sendNotifications
            StudentService.EnrollStudentToCourse(student_Rahul, courses.First(x => x.Id == 1), sendNotifications);
