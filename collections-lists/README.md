## List Collection

This section contains topics about the List collection and solving specific problems regarding Lists in C#.

### Relevant articles:
- [ArrayList in C#](https://code-maze.com/csharp-arraylist/)
- [ArrayList and List Comparasion in C#](https://code-maze.com/csharp-arraylist-vs-list/)
- [Concatenate Lists in C#](https://code-maze.com/csharp-concatenate-lists/)
- [How to Clone a List in C#?](https://code-maze.com/csharp-how-to-clone-a-list/)
- [How to Compare Two Lists Through One Property in C#](https://code-maze.com/csharp-compare-two-lists-through-one-property/)
- [How to Check if Items of a List Exist in Another List in C#](https://code-maze.com/csharp-check-if-items-of-a-list-exist-in-another-list/)
- [Remove Duplicates From a List in C#](https://code-maze.com/remove-duplicates-from-a-list-csharp/)
- [How to Remove Elements From a Generic List in Iteration With C#](https://code-maze.com/csharp-remove-elements-from-list-iteration/)
- [Sort List by Property in the Object in .NET](https://code-maze.com/sort-list-by-object-property-dotnet/)
- [Techniques for Sorting a List in C#](https://code-maze.com/csharp-techniques-for-sorting-list/)
- [How to Convert IAsyncEnumerable to List](https://code-maze.com/csharp-convert-iasyncenumerable-to-list/)
