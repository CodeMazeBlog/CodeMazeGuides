...
namespace ResolvingDependencyInjection.Controllers
{
    ...
    public class WeatherForecastController : ControllerBase
    {
        ...
        [HttpPost(Name = "GetDestinationWeather")]
        public CityForecast Post(TravelDestination model)
        {
            return new CityForecast
            {
                CityName = model.CityName,
                WeatherForecasts = Enumerable.Range(1, model.VacationDays).Select(index => new WeatherForecast
                {
                    Date = DateTime.Now.AddDays(index),
                    TemperatureC = Random.Shared.Next(-20, 55),
                    Summary = Summaries[Random.Shared.Next(Summaries.Length)]
