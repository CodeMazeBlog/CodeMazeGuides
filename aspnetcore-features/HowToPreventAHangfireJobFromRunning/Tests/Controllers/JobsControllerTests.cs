...
public class JobsControllerTests : IClassFixture<ApiApplicationFactory>
{
    private readonly string _baseUri = "http://localhost:5000/api/jobs";

    private readonly HttpClient _client;

    public JobsControllerTests(ApiApplicationFactory factory)
    {
    ...
    public async Task GivenJobsController_WhenCreatingManyOccurrencesOfAGivenJobButUsingSkipConcurrentExecutionAttribute_ThenTheExtraJobsAreBeingSkipped()
    {
        ...
        var statistics = JsonSerializer.Deserialize<StatisticsDto>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
