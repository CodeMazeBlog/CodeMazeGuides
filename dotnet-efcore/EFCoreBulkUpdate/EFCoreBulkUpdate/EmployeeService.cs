...
namespace EFCoreBulkUpdate
{
    public class EmployeeService
    {
        ...
        {
            return await _context.Set<Referee>().AsNoTracking().ToListAsync();
        }
        ...
        {
            await _context.Employees.Where(p => p.YearsExperience > minYearExperience)
                                    .ExecuteDeleteAsync();
        }

        public async Task<int> UpdateAllRefereesYearsExperience()
        {
           return await _context.Set<Referee>().ExecuteUpdateAsync(r => r.SetProperty(t => t.YearsExperience, t => t.YearsExperience + 1));
        }
        public async Task AddReferees(ICollection<Referee> referees)
        {
            _context.Set<Referee>().AddRange(referees);
